...
def eta_wegner(f, Gamma, user_data):
  ...
  '''
  if (np.allclose(eta1B,np.zeros((dim1B,dim1B)))):
    print("eta1B is ZERO")
  else:

    if (np.allclose(eta1B,transpose(eta1B))):
      print("eta1B is symmetric")
    if (np.allclose(eta1B,-transpose(eta1B))):
      print("eta1B is anti-symmetric")

  if (np.allclose(eta2B,transpose(eta2B))):
    print("eta2B is symmetric")
  if (np.allclose(eta2B,-transpose(eta2B))):
    print("eta2B is anti-symmetric")
  '''
...
def main():
  ...
  while solver.successful() and solver.t < sfinal:
    ...
    print("s = {0:5.3f}   E = {1:10.8f}   dE = {2:10.8f}   Gammaod_norm = {3:10.8f}".format(solver.t,E,user_data["dE"],norm_Gammaod))
